...
1. **「ジョブ構成の表示と編集」** ボタンをクリック

1. **「コンテナ、変数とシークレット、接続、セキュリティ」** を展開

1. 「変数とシークレット」で以下を上書き設定：
...
`make run-apply` / `make run-apply-domain` は **terraform apply のように** 動作します。
内部で自動的に Dry-Run を実行し、対象ユーザー一覧を表示した上で確認プロンプトを出します。`yes` を入力した場合のみ本番実行されます。
...
Step 1/2: Dry-Run 実行中... (約30〜60秒)
  ↓
================ Dry-Run 結果 ================
[INFO] 対象ユーザー検出: user:tanaka@customer-a.com
    (予定) + roles/billing.user
    (予定) + roles/billing.viewer
    (予定) + roles/billing.costsManager
    (予定) - roles/billing.admin
[INFO] 対象ユーザー検出: user:sato@customer-a.com
    ...
  ↓
Step 2/2: 上記の対象に対して本番実行します
上記の内容で本当に実行しますか？ (yes/no): yes
  ↓
本番実行 (約30〜60秒)
```

> **所要時間**: Cloud Run Job を2回（Dry-Run + 本番）起動するため合計で1〜2分程度かかります。
> 内容確認のための時間と考えてください。

> **キャンセル時**: `no` を入力した場合や `Ctrl+C` で中断した場合、本番実行は行われません。
> Dry-Run のログだけが GCS に残ります。
...
ログの末尾に、その実行で「何に対して何をしたか」をまとめた実行サマリーが出力されます。
サブアカウントを順に走査する詳細ログは長くなるため、まずここを確認してください。
...
--------------------------------------------------
 ...
 実行者              : tanaka@e-agency.co.jp
 確認サブアカウント  : 82 件
 自社ドメイン除外    : 3 件
 対象外ドメイン除外  : 120 件
 対象ユーザー        : 2 件（変更予定）
--------------------------------------------------
  [012596-1B7FC5-63C5BB] user:sato@customer-a.com
  [013164-AF8D14-E941EF] user:suzuki@customer-a.com
==================================================
```

| 項目 | 意味 |
|---|---|
| モード | `DRY-RUN（変更なし）` か `APPLY（実際に権限を変更）` |
| 対象顧客ドメイン | `DOMAINS` で絞り込んだドメイン。未指定なら全外部ユーザーが対象 |
| 確認サブアカウント | 走査したサブアカウントの総数 |
| 自社ドメイン除外 | `YOUR_DOMAIN` に該当し除外した人数 |
| 対象外ドメイン除外 | 指定ドメイン以外のため除外した人数 |
| 対象ユーザー | 実際に権限変更の対象となった人数と、その一覧 |

> **メモ**
> 除外したユーザーは件数のみを記録し、メールアドレスは出力しません。
> ログの可読性を保つとともに、無関係な他顧客の情報を残さないためです。
> このため、特定の顧客ドメインを指定して実行したログは、その顧客への作業証跡として提示できます。
...
| `INVOKED_BY` | 実行者のメールアドレス（ログ記録用） | `--update-env-vars` で上書き可 |
| `CLOUD_RUN_EXECUTION` | Cloud Run が自動付与する実行ID | 自動（手動上書き不可） |

### 実行者の記録（監査証跡）

「誰がこのシステムを利用したか」を **2系統** で記録しています。

#### ① スクリプトログ（GCS / `make logs`）

`make run` / `make run-apply` などを実行すると、Makefile が `gcloud config get-value account` で実行者のメールアドレスを取得し、Cloud Run Job に `INVOKED_BY` 環境変数として渡します。スクリプトはこれをログヘッダに記録します：

```
[INFO] 2026-06-02 14:23:01 処理開始
[INFO] 2026-06-02 14:23:01 実行者             : tanaka@e-agency.co.jp
[INFO] 2026-06-02 14:23:01 実行ID             : billing-role-sync-job-abc123
[INFO] 2026-06-02 14:23:01 親請求先アカウント : XXXXXX-XXXXXX-XXXXXX
...
```

> Cloud Console UI から直接実行した場合、`INVOKED_BY` を手動で設定しないと `不明` と記録されます。
> その場合でも下記 ② の Cloud Audit Logs に確実に記録されます。

#### ② Cloud Audit Logs（改ざん不可・推奨の監査ソース）

Cloud Run Job の実行は GCP が自動的に Admin Activity 監査ログとして記録します。
ジョブを実行した本人の認証済みメールアドレス（`principalEmail`）が含まれるため、改ざんできず最も信頼できる監査証跡です。

Cloud Console での確認方法：

1. [Cloud Logging](https://console.cloud.google.com/logs) を開く
...
1. 以下のクエリを実行：
   ...
   resource.labels.job_name="billing-role-sync-job"
   protoPayload.methodName="google.cloud.run.v2.Jobs.RunJob"
   ```

1. 各実行の `protoPayload.authenticationInfo.principalEmail` に実行者が記録されています

両方を突き合わせれば、いつ・誰が・どのジョブを・どんな結果で実行したかが完全に追跡できます。
